...
import { citasGetEtapa, citasPrimeraCita, getCheckCitaPrevia } from "../rutasApi.js";
import { useNavigate, useLocation } from "react-router-dom";
import axios from 'axios';

import { Timeline } from 'antd';
import { ClockCircleOutlined, CheckCircleOutlined, CheckSquareOutlined } from '@ant-design/icons';
...
const LineaDelTiempo = () => {
  ...
  const obtenerDatosCitas = async () => {
    try {
      const pacienteResponse = await axios.get(`http://localhost:3001/expedientes/${exp_num}`);
      setNombrePaciente(pacienteResponse.data.nombre);
      const citasResponse = await axios.get(`http://localhost:3001/expedientes/citas/paciente/${exp_num}`);
      ...
      for (let i = 0; i < citasOrdenadas.length; i++) {
        ...
        if (cita.numero_tel_terapeuta) {
          try {
            const terapeutaResponse = await axios.get(`http://localhost:3001/expedientes/usuarios/${cita.numero_tel_terapeuta}`);
        ...
        if (!cita.fecha || !cita.hora) { // Si no hay fecha o hora la cita esta pendiente
          estado = 'pendiente';
          dot = <CheckSquareOutlined style={{ fontSize: '16px', color: '#faad14' }} />;
          color = 'orange';
          label = 'Fecha por asignar';
          descripcion = `Cita Pendiente - Etapa ${cita.etapa || 'A'}: Por agendar fecha y hora`;
        } else {
          const ahora = new Date();
          const fechaCita = new Date(`${cita.fecha} ${cita.hora}`);
          
          if (fechaCita < ahora) { // Cita completada
            estado = 'completada';
            dot = <CheckSquareOutlined style={{ fontSize: '16px', color: '#52c41a' }} />;
            color = 'green';
            descripcion = i === 0 
              ? `Primera Cita - Etapa ${cita.etapa || 'A'}: Evaluación inicial del paciente`
              : `Sesión ${i + 1} - Etapa ${cita.etapa || 'A'}: Sesión de terapia completada`;
          } else if (fechaCita.toDateString() === ahora.toDateString()) {
            // Cita actual (hoy)
            estado = 'actual';
            dot = <CheckSquareOutlined style={{ fontSize: '16px', color: '#1890ff' }} />;
            color = 'blue';
            descripcion = `Cita Actual - Etapa ${cita.etapa || 'A'}: Sesión en progreso`;
          } else {
            estado = 'programada';
            dot = <CheckSquareOutlined style={{ fontSize: '16px', color: '#d9d9d9' }} />;
            color = 'gray';
            descripcion = `Cita Programada - Etapa ${cita.etapa || 'A'}: Próxima sesión`;
          }
        ...
        const item = {
          ...
          children: (
            <div className="timeline-item-content">
              <span>{descripcion}. Terapeuta: {nombreTerapeuta}</span>
              <button className="btn-observaciones" onClick={() => abrirModalObservaciones(cita)}>Observaciones</button>
            </div>
          ),
          dot: dot,
          color: color
    ...
    <div className="main-container">
      ...
      <div className="timeline-container">
        ...
        {timelineItems.length > 0 ? (
          <Timeline mode="left" items={timelineItems} className="timeline-box" />
        ) : (
        ...
        <div className="botones-container">
          <button onClick={SiguienteCitaHandler} className="btn-agendar">Agendar siguiente cita</button>
          <button onClick={SiguienteEtapaHandler} className="btn-siguiente">Elegir siguiente terapeuta</button>
        </div>
